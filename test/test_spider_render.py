...
class TestSpider(fastspider.LightSpider):
	...
	detail_api = "https://mdskip.taobao.com/core/initItemDetail.htm?itemId="
	...
	def parse(self, request, response):
		...
		cookies = cookies2dict(
			"enc=He0FFUfznnffpjS6YSXQiTktURoDYk0on8ZMuumJrVPIKvBtMFgThw7BQ%2F97zg%2BQ7aFqB86eSU%2F5SNj8c%2FThpQ%3D%3D;x5sec=7b226d616c6c64657461696c736b69703b32223a226330303336333135643931663031363236386133656634396666623265323538434c696670496747454f794f6c72502b362b6d397577456f416a443874736d4e41673d3d227d"
		)
		headers = {
		...
		for gid in result[:5]:
			...
			tm_detail_url = self.detail_url + gid
			print(gid)
			yield fastspider.Request(url=tm_detail_api, cookies=cookies, headers=headers, callback=self.parse_detail)

			yield fastspider.Request(url=tm_detail_url, cookies=cookies, headers=headers, callback=self.parse_detail_html)

	def parse_detail(self, request, response):
		text = response.content
		data = json.loads(str(text, encoding='gbk').replace('\n', ''))
		print(data["defaultModel"]["detailPageTipsDO"]["warmingUpHintText"])
		for key, value in data["defaultModel"]["itemPriceResultDO"]["priceInfo"].items():
			print(data["defaultModel"]["itemPriceResultDO"]["priceInfo"][key]["promotionList"][0]["price"])
			break
